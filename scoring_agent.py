...
def score_company(company_id: str):
    ...
    # geography override used for the score above.
    ...
    if priority_geo_matches:
        # geographical_priority is a derived classification, not raw source
        # text - persist the override here too so the "Geography" field shown
        # on the company page matches what actually drove the score, instead
        # of silently disagreeing with it (confirmed real confusion: a company
        # matching the priority list still showed "Low" in Company Research
        # while score_reasoning correctly showed the High-match override).
        update_fields["research_json.geographical_priority"] = "High"
